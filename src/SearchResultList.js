...
export const SearchResultList = ({ searchResults }) => {
  const {showPageItem, setShowPageItem} = React.useContext(ShowPageItemContext);

  const navigate = useNavigate();

  const handleClick = (e) => {
    e.preventDefault();
    setShowPageItem(e.target);
    navigate('/ShowPage');
  }

  return (
      <ShowPageItemContext.Provider value={{showPageItem, setShowPageItem}}>
